...
import ai.bailian.regex.RegexUtils;
...
public class HttpResponse implements Serializable {
    ...
    public HttpResponse(final CloseableHttpResponse response, final HttpContext httpContext, final HttpRequestConfig config) throws HttpException {
        try {
            ...
            //最终请求URL地址
            if (httpContext.getAttribute(HttpCoreContext.HTTP_REQUEST) instanceof HttpUriRequest) {
                HttpUriRequest lastReq = (HttpUriRequest) httpContext.getAttribute(HttpCoreContext.HTTP_REQUEST);
     ...
     * @param encode 编码类型
     */
    public JSON getJSON(final String encode) {
        String response = getString(encode);
        try {
            return (JSON) JSON.parse(response);
        } catch (Exception e) {
            throw new HttpException("Response entity is not a JSON: " + response, e);
        }
    }

    /**
     * 获取JSONObject格式的Entity Body
     */
     ...
     * @param encode 编码类型
     */
    public JSONObject getJSONObject(final String encode) {
        JSON json = getJSON(encode);
        try {
            return (JSONObject) json;
        } catch (Exception e) {
            throw new HttpException("Response entity is not a JSONObject: " + json, e);
        }
    }

    /**
     * 获取JSONArray格式的Entity Body
     */
     ...
     * @param encode 编码类型
     */
    public JSONArray getJSONArray(final String encode) {
        JSON json = getJSON(encode);
        try {
            return (JSONArray) json;
        } catch (Exception e) {
            throw new HttpException("Response entity is not a JSONArray: " + json, e);
        }
    }
     ...
     */
    public ObjectNode getJacksonObject() {
        return getJacksonObject(null);
    }
     ...
     * @param encode 编码类型
     */
    public ObjectNode getJacksonObject(final String encode) {
        String response = getString(encode);
        try {
     ...
     */
    public ArrayNode getJacksonArray() {
        return getJacksonArray(null);
    }
     ...
     * @param encode 编码
     */
    public ArrayNode getJacksonArray(final String encode) {
        String response = getString(encode);
        try {
    ...
    private Charset getCharsetFromHtml(String html) {
        ...
        Elements metas = page.getElementsByTag("head").first()
    ...
    private Charset getCharsetFromContentTypeString(String contentType) {
        ...
        String charset = RegexUtils.getFirstCaptured("charset=([a-zA-Z0-9\\._:\\+-]+)", contentType);
